...
function pauseCassette() {
  cassetteCercle1.classList.remove("animate-spin");
  cassetteCercle2.classList.remove("animate-spin");
}
// Active l'animation de la cassette manuellement
function playCassette() {
  console.log("play");
  cassetteCercle1.classList.add("animate-spin");
  cassetteCercle2.classList.add("animate-spin");
}
...
function nextText() {
  if (textTrack < trackList.length - 1) {
    // index 0 music arr
    textTrack++;
    loadNextTrack(textTrack);
  } else {
...
function nextText2() {
  if (textTrack2 < trackList.length - 1) {
    // index 0 music arr
    textTrack2++;
    loadNextNextTrack(textTrack2);
  } else {
...
if (player) {
  window.addEventListener("keydown", function (event) {
    ...
    if (key === 32) {
      // touche clavier: Barre espace
      playPauseIcon();
      playPauseCassette();
      playPauseMusic();
    } else if (key == 37) {
      // touche clavier: flèche gauche
      prevMusic();
    } else if (key == 39) {
      // touche clavier: flèche droite
      nextMusic();
    } else if (key == 83) {
      // touche clavier: lettre s
      stopMusic();
      stopCassette();
      playIcon();
    }
